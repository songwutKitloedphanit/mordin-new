...
import { LandInputInterface, PublicLandByFarmerInput } from '@/types/Land';
...
export async function createPublicLandByFarmer(data: PublicLandByFarmerInput) {
  try {
    const response = await api.post('/lands/public-by-farmer', data);
    return response.data;
  } catch (error) {
    console.log('Can not create public land', error);
    throw error;
  }
}
