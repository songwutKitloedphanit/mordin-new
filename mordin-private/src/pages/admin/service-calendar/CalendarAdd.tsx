...
const CalendarAdd: React.FC = () => {
  ...
  const handleSubdistrictChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // โจ ถ้ายังไม่มี mapLink โ’ set defaultCenter และ location จากตำบล
      ...
      if (resolvedUrl.includes('goo.gl')) {
        try {
          ...
          if (fullUrl) {
            ...
            console.warn('โ ไม่สามารถ resolve ลิงก์สั้นได้:', rawLink);
            return;
          }
        } catch (err) {
          console.error('๐”ฅ Error while resolving Google Map link:', err);
          return;
        }
      }

      // ๐“ Extract พิกัดจากลิงก์ (full URL เท่านั้น)
  ...
  const handleSubmit = async () => {
    ...
    // ๐ฏ NEW: ฟังก์ชันเลือกพิกัดตามลำดับความสำคัญ
          ...
          <div className="private-card">
            <div className="private-card-header">
              <div className="row row-demo-grid">
                ...
                >
                  <h4 className="private-card-title">เพิ่มปฏิทินให้บริการ</h4>
                </div>
                <div
