...
const LaboratoryEdit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [lab, types] = await Promise.all([
        getLaboratoryById(laboratoryId),
        getAllMachineTypes(),
      ]);
      setMachineType(types);
      setLaboratoryData({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await updateLaboratoryById(laboratoryId, laboratoryData);
      await Swal.fire({
        title: 'สำเร็จ!',
      ...
      const message = err?.response?.data?.message || 'ไม่สามารถแก้ไขข้อมูลได้';
