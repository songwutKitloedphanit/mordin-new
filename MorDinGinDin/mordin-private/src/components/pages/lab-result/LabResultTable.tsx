...
interface LabResultTableProps {
  ...
  handleSubmitPreValue?: (resultValue: ResultInput[]) => void;
}
...
const LabResultTable: React.FC<LabResultTableProps> = ({
  ...
  const saveModalChanges = () => {
    ...
    console.log('Save edited results for', editingSampleCode, changedResults);
    console.log(
      'Save edited dirt weight for',
      editingSampleCode,
      changedDirtWeight
    );
    ...
    if (handleSubmitPreValue) {
      if (changedResults.length > 0 && changedDirtWeight.length > 0) {
        ...
        // เพิ่ม func ที่ทำการอัพเดต DirtWeight ตรงนี้
      } else if (changedDirtWeight.length > 0) {
        // เพิ่ม func ที่ทำการอัพเดต DirtWeight ตรงนี้
      } else {
        handleSubmitPreValue(changedResults);
      }
                                ...
                                return result ? (
                                  <React.Fragment key={result.laboratoryId}>
                                    ...
                                    <td
                                      className="text-left"
                                      style={{
                                        background: result?.preValue
                                          ? result.resultGradeLevel?.color
                                          : '',
                                        minWidth: '0.125vh',
                                      }}
                                    />
