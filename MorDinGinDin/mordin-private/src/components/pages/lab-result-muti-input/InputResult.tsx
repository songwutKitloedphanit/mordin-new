...
// ⬇️ เปลี่ยนคีย์ CRM ให้ผูกกับ standard_certificates (standardId + laboratoryId)
...
type Pair = { short: string; rep: number };
...
interface Props {
  ...
  // ⬇️ crmService.result แต่ละ cell ต้องมี { standardId, laboratoryId, certificateValue, repeatNumber, laboratorySetting }
    ...
    changes.forEach(([key, value, rowId, headerShort]) => {
      ...
      if (value !== currentValue) {
        ...
        const newResultItem: any = {
          ...
          preValue: value,                // ⚠️ ใช้ช่องนี้เป็น carrier แม้ CRM จะหมายถึง certificateValue
        };
    ...
    <>
        ...
        <div className="col-md-12">
          ...
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">ผลวิเคราะห์ (Sample)
          ...
          <div className="card mt-4">
            <div className="card-header">
              <h4 className="card-title">ผลวิเคราะห์ (Blank)
                <span className="text-secondary ms-2">บันทึกเฉพาะที่กรอกค่า</span>
              </h4>
            </div>
            <div className="card-body">
          ...
          <div className="card mt-4">
            <div className="card-header">
              <h4 className="card-title">ผลวิเคราะห์ (Standard / CRM)
                <span className="text-secondary ms-2">บันทึกเฉพาะที่กรอกค่า</span>
              </h4>
            </div>
            <div className="card-body">
              <div className="table-responsive">
                <HotTable
                  ...
                  // ✅ ใช้ keyCrmCert และบันทึกค่าไปยัง certificateValue (ผ่าน preValue carrier)
                  afterChange={(changes, source) => {
                    if (!changes || source === 'loadData') return;
                    const changeList: [string, string, string, string][] = [];
                    changes.forEach(([row, col, _oldVal, newVal]) => {
      ...
      {showConfirm && (
        <ConfirmAlert
          ...
          onConfirm={() => { setShowConfirm(null); localStorage.removeItem('LabResults'); onCancel(); }}
