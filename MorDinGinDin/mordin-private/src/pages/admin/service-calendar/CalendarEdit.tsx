...
const CalendarEdit: React.FC = () => {
  ...
  useEffect(() => {
    const handleMapLink = async () => {
      ...
      if (coords && !isNaN(coords.lat) && !isNaN(coords.lng)) {
        ...
        //console.log('Extracted from mapLink →', { lat, lng });
        setLocation({ lat, lng });
        setDefaultCenter({ lat, lng });
        setCalendarData(prev => ({
          ...prev,
  ...
  const handleSubmit = async () => {
    const validationErrors: Record<string, string> = {}; // Initialize validation errors
          ...
          <div className="card">
            <div className="card-header">
              <div className="row row-demo-grid">
                ...
                >
                  <h4 className="card-title">แก้ไขปฏิทินให้บริการ</h4>
                </div>
                <div
