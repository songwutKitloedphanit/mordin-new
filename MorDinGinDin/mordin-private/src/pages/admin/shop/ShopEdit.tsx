    ...
    const fetchShopData = async () => {
      try {
        ...
        if (shop.latitude && shop.longitude) {
          setLocation({ lat: Number(shop.latitude), lng: Number(shop.longitude) });
          setDefaultCenter({ lat: Number(shop.latitude), lng: Number(shop.longitude) });
        }
        ...
        if (shop.subdistrict) {
          ...
          if (prov) {
            setSelectedProvinceId(prov.code);
            const districts = await getDistrictsByProvinceCode(Number(prov.code));
            setDistrictOptions(districts.map((d: any) => ({ value: d.code, name: d.nameTh })));

            if (dist) {
              setSelectedDistrictId(dist.code);
              const subdistricts = await getSubdistrictsByDistrictCode(Number(dist.code));
              ...
              setSelectedSubdistrictId(sub.code);
            }
          }
        }
      } catch (error) {
          ...
          <div className="card">
            ...
            <div className="card-body">
              <div className="row">
                ...
                {newImage && (
                  <div className="col-12 mb-2 position-relative">
                    <img src={URL.createObjectURL(newImage)} alt="new" width="100%" className="img-thumbnail" />
                    <button
                      type="button"
                      className="btn btn-danger btn-sm position-absolute"
                      style={{ top: 5, right: 20 }}
                    ...
                    <small className="d-block text-center text-success">รูปใหม่</small>
                  </div>
                )}
              </div>
              <div className="form-group mt-3">
                <label htmlFor="shop-images">{existingImage ? 'เปลี่ยนรูปภาพ' : 'เลือกรูปภาพ'}</label>
                <input
                  id="shop-images"
                  type="file"
        ...
        <div className="col-md-6 order-2 order-lg-1">
          <div className="card">
            <div className="card-header d-flex justify-content-between">
              <h4 className="card-title">แก้ไขร้านค้า</h4>
              <GenButtonCircle
                color="btn-info"
                icon="fa fa-list"
                link="/admin/shop"
            ...
            <form onSubmit={handleSubmit} className="card-body">
              ...
              />
              {errors.subdistrict && (
                <div className="invalid-feedback d-block">{errors.subdistrict}</div>
              )}
