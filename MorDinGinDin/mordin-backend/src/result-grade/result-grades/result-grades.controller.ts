...
export class ResultGradesController {
  ...
  @UseGuards(AuthGuard)
  @Post()
  create(@Body() createResultGradeDto: CreateResultGradeDto, @User('sub')  userId: number) {
  ...
  @UseGuards(AuthGuard)
  @Patch(':id')
  update(@Param('id') id: string, @Body() updateResultGradeDto: UpdateResultGradeDto, @User('sub')  userId: number) {
