...
export class ResultGradeLevelsController {
  constructor(private readonly resultGradeLevelsService: ResultGradeLevelsService) {}

  @UseGuards(AuthGuard)
  @Post()
  create(@Body() createResultGradeLevelDto: CreateResultGradeLevelDto, @User('sub')  userId: number) {
    return this.resultGradeLevelsService.create(createResultGradeLevelDto, userId);
  }
  ...
  @UseGuards(AuthGuard)
  @Patch(':id')
  update(@Param('id') id: string, @Body() updateResultGradeLevelDto: UpdateResultGradeLevelDto, @User('sub')  userId: number) {
    return this.resultGradeLevelsService.update(+id, updateResultGradeLevelDto, userId);
  }
