...
export class StandardCalculationService {
    ...
    async calculateResults(results: AnalysisStandardResult[]): Promise<AnalysisStandardResult[]> {
    ...
    private calculateExtractRatio(result: AnalysisStandardResult): number {
        ...
        if (!submitValueExists(result.preValue) || !submitValueExists(extractAmount)) return 0;
