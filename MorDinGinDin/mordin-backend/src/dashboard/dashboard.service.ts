...
export class DashboardService {
  ...
  */
  private async findBookIdsByFilters(filters: DashboardFilterDto): Promise<number[]> {
    ...
    if (filters.serviceTypeId) {
      query.andWhere('book.serviceTypeId = :serviceTypeId', { serviceTypeId: filters.serviceTypeId });
    }
    ...
    if (filters.year) {
      ...
      query.andWhere(`to_timestamp(book.collectSampleAt / 1000) BETWEEN :startDate AND :endDate`, {
    ...
    if (filters.factoryId) {
      query.andWhere('farmer.factoryId = :factoryId', { factoryId: filters.factoryId });
    }

    if (filters.serviceAreaId) {
      query.andWhere('farmer.serviceAreaId = :serviceAreaId', { serviceAreaId: filters.serviceAreaId });
    }

    if (filters.provinceCode) {
      query.andWhere('province.code = :provinceCode', { provinceCode: filters.provinceCode });
    }

    if (filters.districtCode) {
      query.andWhere('district.code = :districtCode', { districtCode: filters.districtCode });
    }

    if (filters.subdistrictCode) {
      query.andWhere('subdistrict.code = :subdistrictCode', { subdistrictCode: filters.subdistrictCode });
    }
  ...
  async getDashboardData(filters: DashboardFilterDto) {
    ...
    if (bookIds.length === 0) {
      return {
        summary: { totalArea: 0, totalFarmer: 0, totalSamples: 0, totalWorkingDays: 0 },
        soilAnalysis: {},
    ...
    const summary = { totalArea: 0, totalFarmer: await this.farmerRepo.count(), totalSamples: bookIds.length, totalWorkingDays: await this.serviceCalendarRepo.count() };
  ...
  async getSoilAnalysisChartData(bookIds: number[]) {
    ...
    const query = this.dataSource
      ...
      .andWhere('grade.parameter != :totalScoreParam', { totalScoreParam: 'Total Score' })
