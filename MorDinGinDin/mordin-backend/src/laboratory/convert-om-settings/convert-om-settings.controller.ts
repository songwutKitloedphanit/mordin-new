...
export class ConvertOmSettingController{
    constructor(private readonly convertOmSettingService: ConvertOmSettingService) {}

    @UseGuards(AuthGuard)
    @Post()
    create(@Body() createConvertOmSettingDto: CreateConvertOmSettingDto, @User('sub')  userId: number) {
        return this.convertOmSettingService.create(createConvertOmSettingDto, userId);
    }
    ...
    update(
        ...
        return this.convertOmSettingService.update(+id, updateConvertOmSettingDto, userId);
    }
