...
export class FarmersService {
  ...
  create(createFarmerDto: CreateFarmerDto, Uid: number) {
  ...
  async update(id: number, updateFarmerDto: UpdateFarmerDto, Uid: number) {
    ...
    Object.assign(farmer, updateFarmerDto, { updateUid: Uid });
  ...
  private maskThaiId(id?: string) {
    const d = this.digits(id);     // d เป็น string เสมอ
  ...
  async publicLogin(dto: FarmerPublicLoginDto): Promise<FarmerPublicProfileDto> {
    ...
    if (dto.loginType === 'thai_id') {
      const idDigits = this.digits(dto.identifier);
      if (!idDigits || idDigits.length !== 13) {
        throw new BadRequestException('กรุณากรอกเลขบัตรประชาชน 13 หลักให้ถูกต้อง');
      }
      qb.andWhere('farmer.thai_national_id = :tid', { tid: idDigits });
    } else {
      // loginType = 'farmer_id'
      const fid = dto.identifier.trim();
      const idDigits = this.digits(fid);
      // กันผู้ใช้กรอกเลข 13 หลักแต่เลือกเป็นหมายเลขเกษตรกร
      if (idDigits.length === 13) {
        throw new BadRequestException('พบรูปแบบเลข 13 หลัก โปรดเลือก "หมายเลขบัตรประชาชน" แทน');
      }
      if (!fid) {
        throw new BadRequestException('กรุณากรอกหมายเลขเกษตรกรให้ถูกต้อง');
      }
      qb.andWhere('farmer.thai_farmer_id = :fid', { fid });
    }

    const matches = await qb.getMany();
    if (matches.length === 0) {
      throw new NotFoundException('ไม่พบข้อมูลเกษตรกรจากเลขที่ให้มา');
    }
    if (matches.length > 1) {
    ...
    const resp: FarmerPublicProfileDto = {
      ...
      thaiNationalIdMasked: this.maskThaiId(farmer.thaiNationalId),
  ...
  async getFarmerSummaryReportsByLand(landId: number): Promise<LandWithSummaryReports[]> {
    ...
    for (const book of books) {
      let result: any;
      if (book.qrCode?.status === SampleStatusEnum.APPROVED && book.sampleCode) {
        try {
          result = await this.bookService.getReports([book.sampleCode]);
        } catch {
          result = null;
        }
      }
  ...
  async generateLandSummaryReportPdf(landId: number, res: Response) {
    const landSummary = await this.getFarmerSummaryReportsByLand(landId);
    if (!landSummary || landSummary.length === 0) {
      res.status(404).setHeader('Content-Type', 'text/plain; charset=utf-8');
      return res.send('No reports found to generate PDF.');
    }

    try {
      const { pdfBuffer, filename } = await this.bookService.buildSummaryReportByLandPdf(landSummary);
      const asciiFilename = `summary-report-${landId}.pdf`;
      ...
      res.send('Cannot generate summary PDF.');
    }
  }
