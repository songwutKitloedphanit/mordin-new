...
export class BooksService {
  ...
  async findReceivedSamplesByServiceCalendarId(serviceCalendarId: number) {
    ...
    const data: { qrCode: QrCode; result: Result[] }[] = [];
   ...
   */
  async selectReceivedBooksByBookId(
    ...
    return this.dataSource.transaction(async manager => {
      ...
      await this.resultService.createBlankResultsForBooksBulk(
        savedBooks,
        manager
      );
  ...
  async settingOwnerData(bookId: number, data: OwnerDataDto, Uid: number) {
    ...
    if (landId) {
      ...
      if (data.latitude !== undefined && data.longitude !== undefined) {
        if (
          land.latitude === null ||
          land.latitude === undefined ||
          land.longitude === null ||
          land.longitude === undefined
        ) {
          Object.assign(land, {
            latitude: isNaN(Number(data.latitude))
              ? undefined
              : parseFloat(data.latitude),
            longitude: isNaN(Number(data.longitude))
              ? undefined
      ...
      book.landId = land.landId;
      book.latitude = land.latitude?.toString();
      book.longitude = land.longitude?.toString();
      book.areaSize = land.areaSize;
    }
  ...
  async getReports(sampleCodes: string[]): Promise<any[]> {
    ...
    if (books.length === 0) {
      throw new NotFoundException(
        `No books found for Sample Codes: ${sampleCodes.join(', ')}`
      );
    }
    ...
    const [
      ...
      this.resultRepo
        ...
        .leftJoinAndSelect('result.laboratorySetting', 'laboratorySetting')
        .leftJoinAndSelect('laboratorySetting.laboratory', 'laboratory')
        .leftJoinAndSelect('laboratory.machineType', 'machineType')
        .leftJoinAndMapOne(
          'result.resultGradeLevel',
          ResultGradeLevel,
          'rgl',
          'rgl.resultGradeId = result.resultGradeId AND rgl.level = result.resultLevel'
        )
        .where('result.bookId IN (:...bookIds)', { bookIds })
        .getMany(),
  ...
  async getSummaryReports(bookId: number): Promise<any[]> {
    ...
    const [results, ferMajorLandScores] = await Promise.all([
      this.resultRepo.find({
        ...
        relations: {
          resultGradeLevel: true,
          laboratorySetting: { laboratory: { machineType: true } },
        },
      }),
      this.ferMajorLandScoreRepo.find({
        where: { bookId: In(bookIds) },
          ...
          <html lang="en">
            ...
            <body>
              ...
              <div class="farmer-info">
                  ...
                  <p class="dot">${report.farmer.firstName}</p>
                  <p class="dot">${report.farmer.lastName}</p>
                  <p></p>
                  <p class="right" style="font-weight: bold;">รหัสตัวอย่าง:</p>
                  <p class="dot">${report.sampleCode}</p>
                  <p style="font-weight: bold;">สถานที่เก็บตัวอย่าง:</p>
                  <p class="dot">${report.land.name}</p>
                  <p class="right" style="font-weight: bold;">รหัสแปลง: </p>
                  <p class="dot"> ${report.land.landCode}</p>
                  <p class="right" style="font-weight: bold;">เขต: </p>
                  <p class="dot"> ${report.farmer.factory.name}</p>
                  <p style="font-weight: bold;">พื้นที่ไร่: </p>
                  <p class="dot"> ${report.land.areaSize}</p>
                  <p class="right" style="font-weight: bold;">อำเภอ: </p>
                  <p class="dot"> ${report.land.subdistrict.district.nameTh}</p>
                  <p class="right" style="font-weight: bold;">จังหวัด: </p>
                  <p class="dot"> ${report.land.subdistrict.district.province.nameTh}</p>
              </div>
            <hr/>
            <div class="farmer-info">
                  <p style="font-weight: bold;">วันที่รับตัวอย่าง:</p>
                  <p class="underline">${formatThaiDateWithOutWeekly(report.sampleReceivedAt)}</p>
                  <p></p>
            ...
            <div class="table-label">
              ...
              <p>ระดับความอุดมสมบูรณ์: ${
                report.ferMajorLandScores.find(
            ...
            <table border="1" cellspacing="0" cellpadding="4" style="border-collapse: collapse; width:100%;">
              <thead>
                <tr style="background-color: #cfe2ff;">
                  ...
                  ${report.serviceType?.serviceCategories
                    .map(
                      cat => `
              ...
              <tbody>
                ${report.usageType
                  .map(usageType => {
                    ...
                    const foundUsage = report.ferMajorLandUsages.find(
                      u =>
                    ...
                    <tr>
                      ...
                      ${report.serviceType?.serviceCategories
                        .map(cat => {
                          const item = report.ferMajorLandUsages.find(
                            i =>
                              i.serviceFertilizerMajorUsage?.usageTypeId ===
                                usageType.usageTypeId &&
                              i.serviceFertilizerMajorUsage
  ...
  async generateSummaryReportByLandPdf(landSummary: any, res: Response) {
    ...
    const labMap: Record<
      number,
      { labName: string; posts: Record<number, number | string> }
    > = {};

    landSummary[0].reports.forEach(report => {
      if (!report.results) return; // ข้ามถ้า results เป็น null/undefined

      report.results?.results?.forEach(r => {
        ...
        const yearSource = r.recordedAt ?? report.date;
        if (!yearSource) return;
        const normalizedYearSource =
          typeof yearSource === 'string' && /^\d+$/.test(yearSource)
            ? Number(yearSource)
            : yearSource;
        const year = new Date(normalizedYearSource).getFullYear();
    ...
    const html = `
          <html lang="en">
            ...
            <body>
              <div class="container">
              <div class="head">
                  <img src="data:image/png;base64,${logoBase64 || imageBase64}"/>
                  <div class="title">รายงานผลการวิเคราะห์ดิน</div>
              </div>
              <p class="address">บริษัท มิตรผลวิจัย พัฒนาอ้อยและน้ำตาล จำกัด ที่อยู่ 399 หมุ่ 1 ถ.ชุมแพ-ภูเขียว ต.โคกสะอาด อ.ภูเขียว จ.ชัยภูมิ 36110</p>
              ...
              <div class="farmer-info">
                  <p style="font-weight: bold;">ชื่อผู้ส่งตัวอย่าง:</p>
                  <p class="dot">${landSummary[0].land.farmer.firstName}</p>
                  <p class="dot">${landSummary[0].land.farmer.lastName}</p>
                  <p></p>
                  <p class="right" style="font-weight: bold;"></p>
                  <p></p>
                  <p style="font-weight: bold;">สถานที่เก็บตัวอย่าง:</p>
                  <p class="dot">${landSummary[0].land.name}</p>
                  <p class="right" style="font-weight: bold;">รหัสแปลง: </p>
                  <p class="dot"> ${landSummary[0].land.landCode ?? '-'}</p>
                  <p class="right" style="font-weight: bold;">เขต: </p>
                  <p class="dot"> ${landSummary[0].land.farmer.factory.name}</p>
                  <p style="font-weight: bold;">พื้นที่ไร่: </p>
                  <p class="dot"> ${landSummary[0].land.areaSize}</p>
                  <p class="right" style="font-weight: bold;">อำเภอ: </p>
                  <p class="dot"> ${landSummary[0].land.subdistrict.district.nameTh}</p>
                  <p class="right" style="font-weight: bold;">จังหวัด: </p>
                  <p class="dot"> ${landSummary[0].land.subdistrict.district.province.nameTh}</p>
                  <p style="font-weight: bold;">เบอร์ติดต่อ: </p>
                  <p class="dot"> ${landSummary[0].land.farmer.phone}</p>
              </div>
            <hr/>
            ...
            <div class="table-label">
              ...
              <table>
                ...
                `
                        ...
                        .join('')
                    : `<tr><td colspan="${Math.max(years.length + 1, 1)}">ไม่มีรายการผลวิเคราะห์</td></tr>`
                }
                </tbody>
    ...
    const filename = `${landSummary[0].land.farmer?.firstName}_${landSummary[0].land.farmer?.lastName}_${landSummary[0].land.landCode}_${landSummary[0].land.farmer?.serviceArea?.code}_${landSummary[0].land.farmer?.factory?.initial}.pdf`;
    archive.append(Buffer.from(pdfBuffer), { name: filename });
  ...
  async createBooking(dto: CreateBookingDto) {
    ...
    if (existingBook) {
      throw new BadRequestException(
        'คุณได้ทำการจองแปลงนี้ในรอบบริการนี้ไปแล้ว'
      );
    }
    ...
    // (Optional
    // แต่ควรทำ)
    const land = await this.landRepo.findOneBy({
      landId: dto.landId,
      farmerId: dto.farmerId,
    });
    if (!land) {
  ...
  async updateBooking(
    bookId: number,
    updateBookingDto: UpdateBookingByFarmerDto
  ) {
    const book = await this.bookRepo.findOne({
      where: { bookId },
      relations: ['qrCode'],
    ...
    if (book.qrCode?.status === SampleStatusEnum.APPROVED) {
      throw new BadRequestException(
        'Cannot change a booking that has already been analyzed'
      );
    }
  ...
  async cancelBooking(bookId: number, farmerId: number): Promise<void> {
    ...
    if (book.qrCode?.status === SampleStatusEnum.APPROVED) {
      throw new BadRequestException(
        'Cannot cancel a booking that has already been analyzed'
      );
    }
