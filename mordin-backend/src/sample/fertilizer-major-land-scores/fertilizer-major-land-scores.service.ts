...
export class FertilizerMajorLandScoresService {
  ...
  private applyCommonFilters(
    ...
    filters: GetGraphFilterDto
  ) {
    if (filters.geographyId)
      query.andWhere('geography.id = :geoId', { geoId: filters.geographyId });
    if (filters.provinceCode)
      query.andWhere('province.code = :provCode', {
        provCode: filters.provinceCode,
      });
    if (filters.districtCode)
      query.andWhere('district.code = :distCode', {
        distCode: filters.districtCode,
      });
    if (filters.subdistrictCode)
      query.andWhere('subdistrict.code = :subdistCode', {
        subdistCode: filters.subdistrictCode,
      });
    if (filters.factoryId)
      query.andWhere('serviceArea.factoryId = :factoryId', {
        factoryId: filters.factoryId,
      });
    if (filters.serviceAreaId)
      query.andWhere('serviceArea.serviceAreaId = :serviceAreaId', {
        serviceAreaId: filters.serviceAreaId,
      });
    if (filters.typeId)
      query.andWhere('book.serviceTypeId = :typeId', {
  ...
  async getGraphData(
    ...
    soilQuery.andWhere('qrCode.status = :status', {
    ...
    majorUsageQuery.andWhere('qrCode.status = :status', {
      status: SampleStatusEnum.APPROVED,
    });
    ...
    minorUsageQuery.andWhere('qrCode.status = :status', {
      status: SampleStatusEnum.APPROVED,
    });
