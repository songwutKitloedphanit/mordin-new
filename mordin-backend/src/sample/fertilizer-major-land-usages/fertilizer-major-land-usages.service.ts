...
export class FertilizerMajorLandUsagesService {
  ...
  async summaryFertilizerMajorLandUsagesBulk(
    ...
    const ferMajorLandUsageRepo = manager.getRepository(
      FertilizerMajorLandUsage
    );
    const ferMajorLandScoreRepo = manager.getRepository(
      FertilizerMajorLandScore
    );
    const soilGradeRepo = manager.getRepository(SoilGrade);
    const serviceFerMajorUsageRepo = manager.getRepository(
    ...
    const allServiceCategoryIds = booksToSummarize.flatMap(item =>
      item.book.serviceType.serviceCategories.map(sc => sc.serviceCategoryId)
    );

    const [allSoilGrades, allServiceFertilizerMajorUsages, allUsageTypes] =
      await Promise.all([
        soilGradeRepo.find({
    ...
    for (const sg of allSoilGrades) {
      if (!soilGradesMap.has(sg.serviceTypeId))
        soilGradesMap.set(sg.serviceTypeId, []);
    ...
    const totalScoreRecordsToCreate: {
      book: Book;
      totalScoreEntity: FertilizerMajorLandScore;
      totalScore: number;
    }[] = [];
    ...
    for (const { book, results } of booksToSummarize) {
      ...
      const soilGradeForTotal = soilGrades.find(
        sg => sg.laboratoryId === null || sg.parameter === 'Total Score'
      );
      ...
      for (const soilGrade of soilGrades) {
        ...
        const refResult = results.find(
          result => result.laboratoryId === soilGrade.laboratoryId
        );
        if (refResult) {
          const selectedLevel =
            ...
            soilGrade.soilGradeLevels[soilGrade.soilGradeLevels.length - 1];

          // [UPDATED] Hardcode filter: Only allow OM, P, K, pH to contribute to Total Score
          const ALLOWED_LABS = ['OM', 'P', 'K', 'pH'];
          const labName = soilGrade.laboratory
            ? soilGrade.laboratory.shortNameBefore
            : 'Unknown';

          if (soilGrade.laboratory && ALLOWED_LABS.includes(labName)) {
            totalScore += selectedLevel.score;
            console.log(
              `   [INCLUDE] Lab: ${labName}, PostValue: ${refResult.postValue}, Score: ${selectedLevel.score}, Current Total: ${totalScore}`
            );
          } else {
            console.log(
              `   [SKIP]    Lab: ${labName}, PostValue: ${refResult.postValue}, Score: ${selectedLevel.score} (Not in allowed list)`
            );
          }
      ...
      const selectedTotalLevel =
        soilGradeForTotal.soilGradeLevels.find(
          level => totalScore <= level.cutoffValue
        ) ||
        soilGradeForTotal.soilGradeLevels[
          soilGradeForTotal.soilGradeLevels.length - 1
        ];
    ...
    for (const { book, totalScoreEntity } of totalScoreRecordsToCreate) {
      const soilGradeForTotal = (
        soilGradesMap.get(book.serviceTypeId) ?? []
      ).find(sg => sg.laboratoryId === null);
      if (!soilGradeForTotal) continue;
      ...
      for (const servCat of book.serviceType.serviceCategories) {
        for (const type of allUsageTypes) {
          ...
          if (serviceFerMajorUsage) {
            const useRate =
              serviceFerMajorUsage.volume *
              serviceFerMajorUsage.fertilizerMajor.quantity;
            const costPerRai =
              (useRate / serviceFerMajorUsage.fertilizerMajor.quantity) *
              serviceFerMajorUsage.fertilizerMajor.price;

            const usageEntity = ferMajorLandUsageRepo.create({
              serviceFertilizerMajorUsageId:
                serviceFerMajorUsage.serviceFertilizerMajorUsageId,
              bookId: book.bookId,
              ...
              gradeText: soilGradeForTotal.soilGradeLevels.find(
                l => l.soilGradeLevelId === totalScoreLevelId
              )?.scoreName,
  ...
  async summaryFertilizerMajorLandUsages(book: Book, results: Result[]) {
    ...
    const serviceFertilizerMajorUsages =
      await this.serviceFerMajorUsageRepo.find({
        where: {
          serviceCategoryId: In(
            serviceCategories.map(cat => cat.serviceCategoryId)
          ),
        },
        relations: {
    ...
    const ferMajorLandScores: FertilizerMajorLandScore[] = [];
    for (const soilGrade of soilGrades) {
      const refResult = results.find(
        result => result.laboratoryId === soilGrade.laboratoryId
      );
      if (refResult) {
        ...
        const savedFerMajorLandScore =
          await this.ferMajorLandScoreRepo.save(ferMajorLandScore);
    ...
    const soilGradeTotal = soilGrades.find(
      soilGrade =>
        soilGrade.laboratoryId === null || soilGrade.parameter === 'Total Score'
    );
    if (!soilGradeTotal) {
    ...
    const selectedTotalLevel =
      matchedTotalLevel ||
      soilGradeTotal.soilGradeLevels[soilGradeTotal.soilGradeLevels.length - 1];
    ...
    const savedFerMajorLandScore = await this.ferMajorLandScoreRepo.save(
      ferMajorLandScoreTotal
    );
    ...
    for (const servCat of serviceCategories) {
      for (const type of usageType) {
        const serviceFerMajorUsage = serviceFertilizerMajorUsages.find(
          usage =>
            usage.serviceCategoryId === servCat.serviceCategoryId &&
